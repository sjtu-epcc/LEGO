...
def load_config(model_args: "ModelArguments") -> "PretrainedConfig":
    ...
    # === NEW: attach skip-layer info to config ===
    if model_args.skip_layers_enabled:
        # store skip-layers into config so the model can read them
        setattr(config, "skip_layers_enabled", True)
        setattr(config, "skip_layers", model_args.skip_layers or [])
    else:
        setattr(config, "skip_layers_enabled", False)
        setattr(config, "skip_layers", [])

    return config


def _disable_grads_for_skipped_layers(model, config):
    if not getattr(config, "skip_layers_enabled", False):
        return

    skip_layers = set(getattr(config, "skip_layers", []))

    for name, param in model.named_parameters():
        # Typical LLaMA block parameter name: "model.layers.16.self_attn.q_proj.weight"
