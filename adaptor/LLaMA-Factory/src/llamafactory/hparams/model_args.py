...
class ModelArguments(
    ...
    # === NEW: enable/disable skip-layer behavior ===
    skip_layers_enabled: bool = field(
        default=False,
        metadata={
            "help": (
                "If True, enable layer skipping during forward pass. "
                "You must implement the logic in modeling_llama.py."
            )
        },
    )

    # === NEW: which layers to skip (only used if enabled) ===
    skip_layers: Optional[List[int]] = field(
        default=None,
        metadata={
            "help": (
                "List of layer indices to skip during forward pass. "
                "Takes effect only when skip_layers_enabled=True."
            )
        },
    )
