...
def get_client(model_str: str, temperature: float = 0.7) -> FunctionCallingLLM:
    ...
    if provider == "openai":
        ...
        return Ollama(model=model_name, temperature=temperature, base_url="http://localhost:11434")
    elif provider == "ollama2":
        from llama_index.llms.ollama import Ollama

        return Ollama(model=model_name, temperature=temperature, base_url="http://localhost:11436")
