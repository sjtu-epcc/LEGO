...
class Player1(Player):
    def __init__(
        ...
        if robot_type == "vision":
            ...
            self.robot.assign_player_num(1)
...
class Player2(Player):
    def __init__(
        ...
        if robot_type == "vision":
            ...
            self.robot.assign_player_num(2)
...
class Game:
    ...
    def run(self):
        ...
        try:
            ...
            # Initialize before the game loop starts 
            previous_p1_health = 160  
            previous_p2_health = 160  
            previous_time = None  # Initialize time tracking
            ...
            while True:
                ...
                current_p1_health = observation["P1"]["health"][0]  
                current_p2_health = observation["P2"]["health"][0]  

                # Detect if the health has been reset (suddenly returning to 160 from a lower health value)
                if (previous_p1_health < 160 and current_p1_health == 160) or (previous_p2_health < 160 and current_p2_health == 160):  
                    # Use the health and time from before the reset as the final values  
                    final_p1_health = previous_p1_health  
                    final_p2_health = previous_p2_health  
                else:  
                    final_p1_health = current_p1_health  
                    final_p2_health = current_p2_health  
                
                # Update the data for the previous frame
                previous_p1_health = current_p1_health  
                previous_p2_health = current_p2_health  

                p1_wins = observation["P1"]["wins"][0]
                p2_wins = observation["P2"]["wins"][0]

                if p1_wins == 1 or p2_wins == 1:
                    print(f"True Final Health - Player1: {final_p1_health}, Player2: {final_p2_health}")  
