...
#pragma warning(disable: 4211 4458 4146)
#include "llama.h"
...
// helpers
//
static void SetDuration(ggml_tensor* tensor)
{
	FString ggml_op;
	switch (tensor->op) {
        case GGML_OP_DUP:
        ...
        case GGML_OP_MUL:
            ...
            break;
        case GGML_OP_UNARY:
            if (ggml_get_unary_op(tensor) == GGML_UNARY_OP_SILU) 
            	ggml_op = FString("silu");
            break;
        case GGML_OP_RMS_NORM:
            ggml_op = FString("rms_norm");
            break;
        case GGML_OP_MUL_MAT:
            ggml_op = FString("mul_mat");
            break;
        case GGML_OP_CPY:
            ggml_op = FString("cpy");
            break;
        case GGML_OP_SOFT_MAX:
            ggml_op = FString("soft_max");
            break;
        case GGML_OP_ROPE:
            ggml_op = FString("rope");
            break;
        default:
            ggml_op = FString("");
    }
	TArray<int> dim;
	for (int i = 0; i < 4; ++i) dim.Push(tensor->src[0]->ne[i]);
	if (tensor->src[1] != NULL) for (int i = 0; i < 4; ++i) dim.Push(tensor->src[1]->ne[i]);
	for (int i = 0; i < 4; ++i) dim.Push(tensor->ne[i]);
	tensor->duration[0] = FDurationPredictor::GetInstance(ggml_op).GetDuration(dim) / 1000.0;
	//std::cout << TCHAR_TO_UTF8(*ggml_op) << " duration: " << tensor->duration[0] << std::endl;
}
...
static void llm_build_kv_store(
	...
	ggml_tensor* tmp1 = ggml_cpy(ctx, k_cur,   k_cache_view);
	SetDuration(tmp1);
    ggml_build_forward_expand(graph, tmp1);
	ggml_tensor* tmp2 = ggml_cpy(ctx, v_cur_t, v_cache_view);
	SetDuration(tmp2);
	ggml_build_forward_expand(graph, tmp2);
}
...
static struct ggml_tensor * llm_build_kqv(
    ...
    if (model.arch == LLM_ARCH_PHI2) {
        ...
        // ref: https://github.com/ggerganov/llamacpp/pull/4490#issuecomment-1859055847
...
// https://github.com/ggerganov/llamacpp/commit/074bea2eb1f1349a0118239c4152914aecaa1be4
...
bool llama_supports_gpu_offload(void) {
    ...
    // Defined when llamacpp is compiled with support for offloading model layers to GPU.
    return true;
#else
    return false;
#endif
